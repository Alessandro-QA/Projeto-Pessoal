...
context('Financeiro', () => {
    context('Dashboard', () => {
        describe(`POST - ${Cypress.env('financeiro')}/v1/Dashboard/ResultadoSafra - Resultado da Safra`, () => {
            it('CT1 - Deve buscar Resultado da Safra - Com saldo a Fixar de Colheitas', () => {
                cy.fixture('financeiro/dashboard/resultadoSafra/payloadCt1.json').then((payload) => {
                    cy.postRequest(`${Cypress.env('baseUrl')}${Cypress.env('financeiro')}//Dashboard/ResultadoSafra`, payload)
                        .then((response) => {
