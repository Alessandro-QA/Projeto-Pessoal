...
context('Financeiro', () => {
    context('Dashboard', () => {
        describe(`POST - ${Cypress.env('financeiro')}/v1/Dashboard/ResultadoSafraAnaliticoCiclos - Resultado da Safra Analítico Ciclos`, () => {
            it('CT1 - Deve buscar Resultado da Safra Analítico Ciclos - Com saldo a Fixar de Colheitas', () => {
                cy.fixture('financeiro/dashboard/resultadoSafraAnaliticoCiclos/payloadCt1.json').then((payload) => {
                    cy.postRequest(`${Cypress.env('baseUrl')}${Cypress.env('financeiro')}//Dashboard/ResultadoSafraAnaliticoCiclos`, payload)
                        .then((response) => {
