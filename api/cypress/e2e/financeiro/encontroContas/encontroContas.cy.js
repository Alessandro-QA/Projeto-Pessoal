...
context('Financeiro', () => {
    context('Encontro Contas', () => {
        describe(`POST - ${Cypress.env('financeiro')}/EncontroContas - Abate um A Pagar por um a Receber`, () => {
            ...
            it('CT2 - Deve listar a agenda A Pagar e a Receber', () => {
                cy.fixture('financeiro/encontroContas/encontroContas/payloadCt2.json').then((payload) => {
                    ...
                    // Para data final adicionamos um mês, visto que o A Receber gerado é automaticamente adicionado com vencimento de um mês após a data do documento
            ...
            it('CT3 - Deve Fazer o Encontro de Contas - Abater o a Pagar com o a Receber', () => {
                cy.fixture('financeiro/encontroContas/encontroContas/payloadCt3.json').then((payload) => {
                    ...
                    cy.postRequest(`${Cypress.env('baseUrl')}${Cypress.env('financeiro')}/EncontroContas`, payload)
                        .then((response) => {
                            expect(response.requestHeaders).to.have.property('x-tenant').to.be.equal(Cypress.env('tenant'));
                            expect(response.status).be.equal(200);
                            expect(response.body).to.exist;
