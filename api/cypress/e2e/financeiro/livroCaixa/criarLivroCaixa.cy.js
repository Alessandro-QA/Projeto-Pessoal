...
context('Financeiro', () => {
    context('Livro Caixa', () => {
        ...
        describe('POST/PATCH/PUT/DELETE - /api/financeiro/v1/LivroCaixa - Criar Lançamento Livro Caixa', () => {
            it('CT1 - Deve criar um lançamento do livro caixa', () => {
                cy.fixture('financeiro/livroCaixa/criarLivroCaixa/payloadCt1.json').then((payload) => {
                    cy.postRequest('/api/financeiro/v1/LivroCaixa', payload)
                        .then((response) => {
            ...
            it('CT2 - Deve editar um lançamento do livro caixa', () => {
                cy.fixture('financeiro/livroCaixa/criarLivroCaixa/payloadCt2.json').then((payload) => {
                    ...
                    cy.putRequest('/api/financeiro/v1/LivroCaixa', payload)
                        .then((response) => {
                            expect(response.requestHeaders).to.have.property('x-tenant').to.be.equal(Cypress.env('tenant'))
                            expect(response.status).be.equal(200)
                            expect(response.body).be.not.null
            ...
            it('CT3 - Deve patchear um lançamento do livro caixa', () => {
                cy.fixture('financeiro/livroCaixa/criarLivroCaixa/payloadCt3.json').then((payload) => {
                    cy.patchRequest(`/api/financeiro/v1/LivroCaixa/${livroCaixaID}`, payload).then((response) => {
            ...
            it('CT4 - Deve Redefinir um lançamento do livro caixa', () => {
                cy.postRequest(`/api/financeiro/v1/LivroCaixa/Reset/${livroCaixaID}`, livroCaixaID).then((response) => {
                    expect(response.requestHeaders).to.have.property('x-tenant').to.be.equal(Cypress.env('tenant'))
                    expect(response.status).be.equal(200)
                    expect(response.body).be.not.null
                    expect(response.body).to.exist
            ...
            it('CT5 - Deve Deletar Lançamento', () => {
                cy.deleteRequest('/api/financeiro/v1/LivroCaixa', livroCaixaID).then((response) => {
                    expect(response.status).be.equal(200)
                })
            })
