...
context('Financeiro', () => {
    context('Documento', () => {
        ...
        describe(`POST/PUT/DELETE - ${Cypress.env('financeiro')}/Documento - Criar Documento`, () => {
            ...
            it('CT3 - Deve Deletar Documento', () => {
                cy.deleteRequest(`${Cypress.env('baseUrl')}${Cypress.env('financeiro')}/Documento`, documentoID).then((response) => {
                    expect(response.status).to.be.equal(200);
                });
            });
