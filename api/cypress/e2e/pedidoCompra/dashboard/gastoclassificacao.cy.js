            ...
            cy.fixture('pedidoCompra/dashboard/gastoclassificacao/paramsCt2.json').then((params) => {
                ...
                cy.getRequestWithParams(`${Cypress.env('baseUrl')}${Cypress.env('pedidoCompra')}/Dashboard/GastoClassificacao`, params)
                    .then((response) => {
                        ...
                        // Verifica se contém exatamente uma categoria 'Grãos' com os valores esperados
                        const categoriaGrãos = response.body.find(categoria => categoria.nomeCategoria === 'Grãos');
                        expect(categoriaGrãos).to.exist;
                        expect(categoriaGrãos).to.have.property('valorMateriais').that.is.a('number');
                        expect(categoriaGrãos).to.have.property('porcentagem').that.is.a('number');
                    });
            });
        });
    });
